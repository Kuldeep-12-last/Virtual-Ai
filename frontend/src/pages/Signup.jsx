...
function Signup() {
  ...
  const HandleSignup = async (event) => {
    ...
    try {
      ...
      console.log(err.response?.data?.message || err.message || "Unknown error");
    ...
    >
      ...
      >
        <h1 className="text-white text-xl sm:text-2xl font-semibold text-center">
          Registration to{" "}
          <span className="text-blue-700 font-semibold text-xl sm:text-3xl">
            Virtual Assistant
          </span>
        </h1>
        ...
        {ERR && (
          <p className="text-red-600 text-sm sm:text-base text-center">*{ERR}</p>
        )}
